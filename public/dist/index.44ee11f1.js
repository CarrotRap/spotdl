...
})({"giyig":[function(require,module,exports) {
"use strict";
var HMR_HOST = null;
var HMR_PORT = null;
...
Vue.createApp({
    ...
    methods: {
        ...
        getArtist (track) {
            var artist = "";
            for(var i = 0; i < track.artists.length; i++)artist = artist + track.artists[i].name + (i + 1 !== track.artists.length ? ', ' : '');
            return artist;
        },
        develop (playlist) {
            if (this.playlistOpen !== playlist.id) fetch(playlist.tracks.href, {
                headers: {
        ...
        downloadPlaylist (playlist) {
            for (const track of this.trackPlaylistOpen)document.getElementById(track.track.id).click();
        }
    },
    mounted () {
        if (accessToken) fetch('https://api.spotify.com/v1/me/playlists', {
            headers: {
                'Content-Type': 'application/json',
                'Authorization': 'Bearer ' + accessToken
            }
...
},{}]},["giyig","7SwCM"], "7SwCM", "parcelRequire94c2")

//# sourceMappingURL=index.44ee11f1.js.map
